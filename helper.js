		...
		.then((user) => {
			if (user && user.isEnrolled()) {
				...
				var userJSON = {
					...
					member = new User(userJSON, client);
...
module.exports.processProposal = function(chain, results, proposalType) {
	...
	//console.log('deploy proposalResponses:'+JSON.stringify(proposalResponses));
...
/*module.exports.getArgsInvoke = function(chaincodeArgs) {
	var args = [];
	console.log("input arg length = " + chaincodeArgs.length);
	for (var i = 0; i < chaincodeArgs.length; i++) {
		args.push(chaincodeArgs[i]);
	}
