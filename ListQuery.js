...
module.exports.getContractList = function(entityId){

var arr = [];
arr.push("contractList")
arr.push(entityId)
...
return hfc.newDefaultKeyValueStore({
	...
	function(admin) {
		...
		logger.info('Executing Query' , config.queryRequest.args);
		var targets = [];
		for (var i = 0; i < config.peers.length; i++) {
	...
	function(response_payloads) {
		for (let i = 0; i < response_payloads.length; i++) {
			//logger.info('############### Query results after :qlt is %j', i, response_payloads[i].toString('utf8'));
		}
